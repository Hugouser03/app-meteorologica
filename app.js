...
document.getElementById('search-btn').addEventListener('click', async () => {
    ...
    weatherCard.innerHTML = `<p>Consultando base de datos geográfica y meteorológica...</p>`;
    ...
    try {
        ...
        if (data.length === 0) {
            weatherCard.innerHTML = `<p style="color:#fee2e2;">No se encontraron resultados para esa ubicación.</p>`;
            return;
        }
        ...
        marker.bindPopup(`<strong>${city}</strong><br>${lugar.display_name}`).openPopup();
        ...
        console.error(error);
        weatherCard.innerHTML = `<p style="color:#fee2e2;">Error al consultar la información.</p>`;
    }
});